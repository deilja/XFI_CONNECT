...
def db() -> sqlite3.Connection:
    ...
    c.executescript("""
    CREATE TABLE IF NOT EXISTS users(id INTEGER PRIMARY KEY,email TEXT UNIQUE NOT NULL,password_hash TEXT,telegram_id INTEGER,telegram_username TEXT,created_at TEXT NOT NULL,balance REAL NOT NULL DEFAULT 0,status TEXT NOT NULL DEFAULT 'active');
    CREATE TABLE IF NOT EXISTS auth_codes(id INTEGER PRIMARY KEY,email TEXT NOT NULL,code TEXT NOT NULL,expires_at TEXT NOT NULL,used INTEGER NOT NULL DEFAULT 0,created_at TEXT NOT NULL);
    CREATE TABLE IF NOT EXISTS subscriptions(id INTEGER PRIMARY KEY,user_id INTEGER NOT NULL,tariff_id INTEGER,status TEXT NOT NULL,expire_at TEXT,subscription_url TEXT,client_uuid TEXT,created_at TEXT NOT NULL);
    CREATE TABLE IF NOT EXISTS payments(id INTEGER PRIMARY KEY,user_id INTEGER NOT NULL,tariff_id INTEGER,amount REAL NOT NULL,status TEXT NOT NULL,provider TEXT NOT NULL,external_id TEXT UNIQUE,payment_url TEXT,created_at TEXT NOT NULL);
    CREATE TABLE IF NOT EXISTS telegram_links(id INTEGER PRIMARY KEY,user_id INTEGER NOT NULL,code TEXT UNIQUE NOT NULL,expires_at TEXT NOT NULL,used INTEGER NOT NULL DEFAULT 0);
    """)
    c.commit()
    return c
...
def current_user(request: Request):
    ...
    try:
        ...
        return None
    c = db(); row = c.execute("SELECT * FROM users WHERE id=? AND status='active'", (int(data["uid"]),)).fetchone(); c.close()
    return row
...
def tariffs() -> list[dict[str, Any]]:
    ...
    for t in native_tariffs():
        ...
        out.append({"id": int(t["id"]), "name": str(t.get("name") or "Тариф"), "price": minor / 100 if minor else float(t.get("price_rub") or 0), "duration_days": int(t.get("duration_days") or 30), "traffic_gb": int(t.get("traffic_limit_gb") or 0) or None, "device_limit": int(t.get("max_ips") or 1), "active": bool(t.get("is_active", 1))})
    return [x for x in out if x["active"]] or [{"id": 1, "name": "1 месяц", "price": 299, "duration_days": 30, "traffic_gb": None, "device_limit": 2, "active": True}]
...
async def xui_add_client(client_uuid: str, email: str, expire_ms: int, total_gb: int | None):
    ...
    payload = {"id": int(XUI_INBOUND_ID), "settings": json.dumps({"clients": [{"id": client_uuid, "email": email, "enable": True, "expiryTime": expire_ms, "totalGB": int(total_gb or 0), "limitIp": 0}]})}
...
def index(request: Request):
    return RedirectResponse("/cabinet", 302) if current_user(request) else HTMLResponse(PAGE_HOME)

@app.get("/login", response_class=HTMLResponse)
def login_page(): return HTMLResponse(PAGE_LOGIN)

@app.get("/cabinet", response_class=HTMLResponse)
def cabinet_page(request: Request): return HTMLResponse(PAGE_CABINET) if current_user(request) else RedirectResponse("/login", 302)

@app.get("/health")
def health(): return {"ok": True, "service": "xfi-connect-web-cabinet", "version": "2.1.0"}

@app.post("/api/auth/request-code")
async def request_code(request: Request):
    data = await request.json(); email = str(data.get("email", "")).strip().lower()
    if "@" not in email or len(email) > 254: return JSONResponse({"detail": "Некорректный email"}, 400)
    c = db(); cutoff = (now() - timedelta(minutes=10)).isoformat(); n = c.execute("SELECT COUNT(*) FROM auth_codes WHERE email=? AND created_at>?", (email, cutoff)).fetchone()[0]
    if n >= 5: c.close(); return JSONResponse({"detail": "Слишком много запросов"}, 429)
    code = f"{secrets.randbelow(1000000):06d}"; c.execute("INSERT INTO auth_codes(email,code,expires_at,created_at) VALUES(?,?,?,?)", (email, code, (now()+timedelta(seconds=CODE_TTL)).isoformat(), now().isoformat())); c.commit(); c.close(); send_code(email, code)
    return {"message": "Код отправлен на почту"}

@app.post("/api/auth/verify-code")
async def verify_code(request: Request, response: Response):
    data = await request.json(); email = str(data.get("email", "")).strip().lower(); code = str(data.get("code", "")).strip(); c = db(); row = c.execute("SELECT * FROM auth_codes WHERE email=? AND code=? AND used=0 ORDER BY id DESC LIMIT 1", (email, code)).fetchone()
    if not row or datetime.fromisoformat(row["expires_at"]) < now(): c.close(); return JSONResponse({"detail": "Неверный или истёкший код"}, 400)
    c.execute("UPDATE auth_codes SET used=1 WHERE id=?", (row["id"],)); u = c.execute("SELECT * FROM users WHERE email=?", (email,)).fetchone()
    if not u: c.execute("INSERT INTO users(email,created_at) VALUES(?,?)", (email, now().isoformat())); uid = c.execute("SELECT last_insert_rowid()").fetchone()[0]
    else: uid = u["id"]
    c.commit(); c.close(); response.set_cookie("xfi_session", token(uid), httponly=True, secure=COOKIE_SECURE, samesite="lax", max_age=30*86400); return {"ok": True}

@app.post("/api/auth/logout")
def logout(response: Response): response.delete_cookie("xfi_session"); return {"ok": True}

@app.get("/api/auth/me")
def me(request: Request):
    u = current_user(request)
    return json_user(u) if u else JSONResponse({"detail": "Не авторизован"}, 401)


def json_user(u):
    return {"id": u["id"], "email": u["email"], "telegram_id": u["telegram_id"], "telegram_username": u["telegram_username"], "balance": u["balance"], "status": u["status"]}

@app.get("/api/cabinet/tariffs")
def get_tariffs(): return tariffs()
...
def get_subscriptions(request: Request):
    ...
    c=db(); rows=c.execute("SELECT * FROM subscriptions WHERE user_id=? ORDER BY id DESC",(u["id"],)).fetchall(); c.close(); return [dict(x) for x in rows]
...
def active(request: Request):
    ...
    if not r or (r["expire_at"] and datetime.fromisoformat(r["expire_at"])<=now()): return {"has_active":False,"subscription":None}
...
async def create_payment(request: Request):
    ...
    external=secrets.token_urlsafe(18); c=db(); c.execute("INSERT INTO payments(user_id,tariff_id,amount,status,provider,external_id,created_at) VALUES(?,?,?,?,?,?,?)",(u["id"],tid,t["price"],"pending","platega",external,now().isoformat())); pid=c.execute("SELECT last_insert_rowid()").fetchone()[0]; c.commit(); c.close()
    if not PLATEGA_MERCHANT or not PLATEGA_SECRET:return {"payment_id":pid,"status":"pending","payment_url":None,"message":"Platega не настроена"}
    headers={"X-MerchantId":PLATEGA_MERCHANT,"X-Secret":PLATEGA_SECRET}; body={"paymentMethod":"SBP","paymentDetails":{"amount":t["price"],"currency":"RUB"},"description":f"{APP}: {t['name']}","return":PLATEGA_RETURN,"payload":external}
    async with httpx.AsyncClient(timeout=XUI_TIMEOUT) as client:
        ...
        if r.status_code>=400:return JSONResponse({"detail":"Не удалось создать платёж"},502)
        data=r.json()
    url=data.get("redirect") or data.get("paymentUrl") or data.get("url"); c=db(); c.execute("UPDATE payments SET payment_url=? WHERE id=?",(url,pid)); c.commit(); c.close(); return {"payment_id":pid,"status":"pending","payment_url":url,"external_id":external}
...
async def payment_webhook(request: Request):
    ...
    c.execute("UPDATE payments SET status=? WHERE id=?",("paid" if status=="CONFIRMED" else status.lower(),p["id"])); c.commit(); c.close(); return {"ok":True}

@app.post("/api/telegram/link")
def telegram_link(request: Request):
    u=current_user(request)
    if not u:return JSONResponse({"detail":"Не авторизован"},401)
    code=secrets.token_urlsafe(8); exp=now()+timedelta(minutes=15); c=db(); c.execute("INSERT INTO telegram_links(user_id,code,expires_at) VALUES(?,?,?)",(u["id"],code,exp.isoformat())); c.commit(); c.close(); bot=os.getenv("TELEGRAM_BOT_USERNAME","XFI_VPN_bot"); return {"code":code,"expires_at":exp.isoformat(),"bot_link":f"https://t.me/{bot}?start=web_{code}"}

@app.post("/api/telegram/confirm")
async def telegram_confirm(request: Request):
    secret=request.headers.get("X-XFI-Secret","")
    if not secrets.compare_digest(secret,os.getenv("XFI_WEB_SECRET","")):return JSONResponse({"detail":"Forbidden"},403)
    data=await request.json(); code=str(data.get("code","")); tg=int(data.get("telegram_id")); c=db(); link=c.execute("SELECT * FROM telegram_links WHERE code=? AND used=0",(code,)).fetchone()
...
async def trial(request: Request):
    ...
    url=subscription_url(client_uuid); c.execute("INSERT INTO subscriptions(user_id,tariff_id,status,expire_at,subscription_url,client_uuid,created_at) VALUES(?,?,?,?,?,?,?)",(u["id"],-1,"active",exp.isoformat(),url,client_uuid,now().isoformat())); c.commit(); c.close(); return {"ok":True,"message":"Пробная подписка активирована","subscription_url":url,"expire_at":exp.isoformat()}
