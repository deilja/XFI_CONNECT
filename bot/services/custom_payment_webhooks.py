...
def get_custom_payment_webhook_port() -> int:
    try: port = int(get_setting(WEBHOOKS_PORT_SETTING, str(DEFAULT_WEBHOOKS_PORT)) or DEFAULT_WEBHOOKS_PORT)
    except (TypeError, ValueError): return DEFAULT_WEBHOOKS_PORT
...
async def _trial_vpn_webhook_handler(request: web.Request) -> web.Response:
    ...
    except (json.JSONDecodeError, ValueError): return web.json_response({'ok': False, 'reason': 'invalid_json'}, status=400)
    ...
    try:
        ...
        for admin_id in get_ai_admin_ids():
            ...
            except Exception as exc: logger.warning("Trial VPN notification admin=%s: %s", admin_id, exc)
    except Exception as exc: logger.warning("Trial VPN notification failed: %s", exc)
...
async def _payment_webhook_handler(request: web.Request) -> web.Response:
    ...
    try: provider = get_payment_provider(provider_id)
    except ValueError: provider = None
    if provider is None or provider.webhook_handler is None: return web.json_response({'ok': False, 'reason': 'provider_not_found'}, status=404)
    provided_secret = request.headers.get(WEBHOOK_SECRET_HEADER) or request.query.get('secret')
    if not validate_payment_webhook_secret(provider.provider_id, provided_secret): return web.json_response({'ok': False, 'reason': 'forbidden'}, status=403)
    try:
...
async def _build_request_context(request: web.Request, provider_id: str) -> dict[str, Any]:
    body = await request.read(); body_text = body.decode('utf-8', errors='replace'); content_type = (request.content_type or '').casefold(); json_payload = None; form_payload: dict[str, Any] = {}
    if content_type == 'application/json' and body_text:
        try: json_payload = json.loads(body_text)
        except json.JSONDecodeError: pass
    elif content_type == 'application/x-www-form-urlencoded' and body_text:
        parsed = parse_qs(body_text, keep_blank_values=True); form_payload = {key: values[0] if len(values) == 1 else values for key, values in parsed.items()}
    return {'provider_id': provider_id, 'method': request.method, 'path': request.path, 'query': dict(request.query), 'headers': dict(request.headers), 'body': body_text, 'body_bytes': body, 'json': json_payload, 'form': form_payload, 'remote': request.remote, 'content_type': request.content_type}

def _public_webhook_response(result: dict[str, Any]) -> dict[str, Any]:
    response = {'ok': bool(result.get('ok')), 'status': result.get('status'), 'ignored': bool(result.get('ignored')), 'completed': bool(result.get('completed')), 'processed_now': bool(result.get('processed_now'))}
    if result.get('order_id'): response['order_id'] = result.get('order_id')
    if result.get('reason'): response['reason'] = result.get('reason')
    return response

def _webhook_http_status(result: dict[str, Any]) -> int:
    try: status = int(result.get('http_status') or (200 if result.get('ok') else 400))
...
__all__ = ['DEFAULT_WEBHOOKS_HOST','DEFAULT_WEBHOOKS_PATH_PREFIX','DEFAULT_WEBHOOKS_PORT','WEBHOOKS_ENABLED_SETTING','WEBHOOKS_HOST_SETTING','WEBHOOKS_PATH_PREFIX_SETTING','WEBHOOKS_PORT_SETTING','WEBHOOK_SECRET_HEADER','BOT_APP_KEY','CustomPaymentWebhookServer','create_custom_payment_webhook_app','get_custom_payment_webhook_host','get_custom_payment_webhook_path_prefix','get_custom_payment_webhook_port','is_custom_payment_webhook_server_enabled','start_custom_payment_webhook_server']
