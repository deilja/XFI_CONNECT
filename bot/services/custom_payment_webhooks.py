...
async def _trial_vpn_webhook_handler(request: web.Request) -> web.Response:
    """Receives Trial VPN events without requiring another Telegram bot token."""
    secret = os.getenv(TRIAL_WEBHOOK_SECRET_ENV, '').strip()
    if not secret:
        logger.warning("Trial VPN webhook rejected: %s is not configured", TRIAL_WEBHOOK_SECRET_ENV)
    ...
    try:
        ...
        text = (
            "<b>XFI CONNECT — новый VPN-тест</b>\n\n"
            f"Срок: {hours} ч.\n"
            f"Истекает: {expires}\n"
        ...
        for admin_id in get_ai_admin_ids():
            try:
                ...
                logger.warning("Не удалось отправить Trial VPN событие admin=%s: %s", admin_id, exc)
    except Exception as exc:
...
def _webhook_http_status(result: dict[str, Any]) -> int:
    try: status = int(result.get('http_status') or 200 if result.get('ok') else result.get('http_status') or 400)
