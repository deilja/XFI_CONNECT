...
This module intentionally contains no legacy updater implementation. Public
helpers are retained because existing handlers/extensions import them.
"""
...
def get_current_commit(*, full: bool = False) -> Optional[str]:
    try:
        return _commit(_root()) if full else _commit(_root())[:8]
    except Exception:
        return None
...
def set_remote_url(url: str) -> Tuple[bool, str]:
    """Set origin only when it resolves to the fixed XFI CONNECT repository."""
...
def _apply_target(target: str, *, mode: str, actor: Optional[str]) -> Tuple[bool, str]:
    ...
    try:
        with update_operation_lock(root):
            ...
            source = _commit(root)
            snapshot = create_pre_update_snapshot(update_mode=mode, requested_target=target, actor=actor, project_root=root)
            rescue = None
            try:
                _checked(["git", "reset", "--hard", target], root, stage="Applying XFI CONNECT update", timeout=120)
                try:
                    _checked([sys.executable, "-m", "pip", "install", "--upgrade", "-r", str(root / "requirements.txt")], root,
                             stage="Installing XFI CONNECT dependencies", timeout=600)
                except Exception:
                    ...
                    raise
                finalize_snapshot_after_git(snapshot, git_succeeded=True, project_root=root)
                info = get_last_commit_info("HEAD")
                return True, f"✅ XFI CONNECT обновлён.\n\n<pre>{info}</pre>"
            except Exception:
                raise
...
def pull_to_commit(commit_hash: str, *, update_mode: str = "admin_target_commit", actor: Optional[str] = None) -> Tuple[bool, str]:
    if not isinstance(commit_hash, str) or not __import__("re").fullmatch(r"[0-9a-fA-F]{40,64}", commit_hash):
        return False, "❌ Некорректный идентификатор коммита"
    root = _root()
    if not _run(["git", "cat-file", "-e", f"{commit_hash}^{{commit}}"], root, 30).returncode == 0:
